 ...
 * 하지만 Controller 는 Front 와 연개되어 매우 빈번하게 호출되는 클래스이다
 * Controller 클래스에 많은 요소들이 첨가되어 있으면 아무래도 성능상에
 * 이슈가 발생할 수 있다
 * 
 * 그래서 외부 다른 요소, 도구를 사용하여 연상을 수행이 필요한 경우
 * 중간에 Service 를 두고, 그곳에서 처리하는 것이 다소간에 연산의 이점이 있다.
 * 
 * 만약 매우 단순하게 Controller 와 Dao 간에 1:1 로 매칭되어 진행이 될수 있는 경우라도
 * 가급적 중간에 Service 가 Toss 역할을 수행하도록 하는 것을 권강한다.
 */
