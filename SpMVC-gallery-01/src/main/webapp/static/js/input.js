...
base64 제약 사항 파일 크기가 상당히 큼
jpeg는 다소 양호하나 png 나 백터 파일은 여러가지 이슈가 있음.
파일의 크기 문제로 업로드 , DB 저장 등에서 문제를 일으킬 수 있다. 
다만, DB에 파일을 저장하므로써 별도의 이미지를 보관하는 방식에 비해 다소 유리한 점도 있음. 

BASE64로 변환된 파일을 압축하여 jpeg로 변환하면 용량문제를 해결할 수 있다.
  */
  const ecodeImageFileAsBase64 = async (image) => {
    return new Promise((resoleve, _) => {
      ...
      reader.onloadend = () => {
        ...
        myImage.onload = (e) => {
          //객체 (myimage) 에 이미지가 load 되면 화면에
          //2d 가상 canvas를 생성하고 그 canvas에 myimage에 담긴 이미지를 그려라
          ...
          //canvas에 그려진 이미지를 jpeg 로 변환하고 크기를 0.5배
          let point = 1.0;
          ...
          if (reSizeImage.length > imageSize) {
            alert("이미지 너무 큼 업로드 ㄴㄴ");
            return false;
          }
          resoleve(reSizeImage);
        };
      };
