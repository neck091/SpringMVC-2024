...
const onDelete = ()=>{
	if(confirm("정말 삭제할까요")){
		document.location.replace("${rootpath}/iolist/iolist/delete/${IO.io_seq}")
	}
...
<div class="iolist detail">
	<div>
		<strong>거래일자</strong><span>${IO.io_date}</span>
	</div>
	<div>
		<strong>거래시각</strong><span>${IO.io_time}</span>
	</div>
	<div>
		<strong>상품명</strong><span>${IO.io_pname}</span>
	</div>
	<div>
		<strong>거래구분</strong><span>${IO.io_inout}</span>
	</div>
	<div>
