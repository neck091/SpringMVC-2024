...
public class FileUploadServiceImpl implements FileUploadService{
	...
	public FileUploadServiceImpl(ServletContext context) {
		...
		//tomcat 폴더가 아닌 servr의 로컬 스토리지의 임의의 폴더
	...
	public String fileUpload(MultipartFile file) throws Exception {
		 ...
		 * Server 가 정상적으로 실행되는 동안에는 아무런 문제없이 이 폴더에
		 * 파일을 저장하거나, 읽기가 가능하다.
...
//		log.debug("업로드 폴더 {}",folder);
		...
		// 문자열로 된 폴더 정보를 Java에서 사용하기 위해 객체로 변환하기
		...
		/*
		 * 같은 이름의 파일을 업로드 하여 기존에 업로드된 파일을 변형하는
		 * 공격을 막기 위하여 파일이름 앞에  UUID를 부착하여 저장되는 파일의 이름을 변형
		 */
		String uuid = UUID.randomUUID().toString();
		String upLoadFileName = String.format( "%s-%s", uuid,orignalFileName);
		
		// 폴더 이름 +파일이름 을 결합하여 업로드할 파일 정보를 생성하여
		// uploadFile 객체를 저장
		...
		//실제 저장된 업로드 파일의 이름을 return
		return upLoadFileName;
	}
	
	
	/*
	 * 업로드된 멀티파일을 List type의 MultipartFile 로 분해하기
	 * 업로드된 멀티파일을 추출하기 위해서 ㅍㅏ일즈 getFiles(이름) 을 사용
	 * form input tag 에 붙여진걸 사용
	 */
