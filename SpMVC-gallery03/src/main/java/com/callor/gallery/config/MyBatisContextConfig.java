...
public class MyBatisContextConfig {
	...
	/*
	 * javacode에서 /web-inf 또는 src/main/resource 등의 폴더에 있는 xml 등의 파일에 접근하기 위한 도구
	 */
	...
	public SqlSessionFactoryBean sessionFactoryBean() {
		...
		/*
		 * tomcat 서버의 저장소에서 ~~/WEB-INF/spring/mapper 폴더에서 *-mapper.xml 파일들의 정보를 가져와서
		 * MapperLocation 속성에 등록
		 */

		try {
			bean.setMapperLocations(context.getResources("/WEB-INF/spring/mapper/*-mapper.xml"));

			// ~~/src/main/resurces 폴더의 mapper 폴더에서 xml 을 참조하고 싶을 때
			//context.getResource("/WEB-INF/spring/mapper/*-mapper.xml");
